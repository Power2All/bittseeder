...
pub fn setup_handlers(
    ...
    dc.on_open(Box::new(|| {
        log::info!("[RTC] Data channel opened");
        Box::pin(async {})
    }));
    dc.on_close(Box::new(|| {
        log::info!("[RTC] Data channel closed");
        Box::pin(async {})
    }));
    dc.on_error(Box::new(|e| {
        log::error!("[RTC] Data channel error: {}", e);
        Box::pin(async {})
    }));
    dc.on_message(Box::new(move |msg: DataChannelMessage| {
        ...
        Box::pin(async move {
            handle_rtc_message(data, dc2, ti2, up2, rl2).await;
        })
    }));
}

async fn handle_rtc_message(
    data: Bytes,
    dc: Arc<RTCDataChannel>,
    torrent_info: Arc<TorrentInfo>,
    uploaded: Arc<AtomicU64>,
    ...
    log::debug!("[RTC] Piece request: {}", piece_index);
        ...
        Ok(piece_data) => {
            ...
            match bytes_sent {
                Ok(n) => {
                    ...
                    log::debug!("[RTC] Sent piece {} ({} bytes)", piece_index, n);
                }
                Err(e) => {
                    log::error!("[RTC] Send error for piece {}: {}", piece_index, e);
                }
            }
        }
        Err(e) => {
            log::error!("[RTC] Failed to read piece {}: {}", piece_index, e);
        }
    }
}
